...
void *cam_thread(void *arg)
{
    ...
    while(!pglobal->stop) {
        ...
        if(pcontext->videoIn->formatIn != V4L2_PIX_FMT_MJPEG) {
            ...
            pglobal->in[pcontext->id].size = compress_yuv_to_jpeg(pcontext->videoIn, pglobal->in[pcontext->id].buf, pcontext->videoIn->framesizeIn, gquality, pcontext->videoIn->formatIn);
        } else {
